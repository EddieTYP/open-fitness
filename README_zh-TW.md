...
平日可以直接用適合手機操作的網頁介面記錄，也可以匯入固定格式的資料。如果想用
相片，或直接打幾句話來記錄，亦可連接 AI 助手；即使不用 AI，網頁介面也能獨立
運作。
...
[新使用者入門指南](docs/ONBOARDING.md)（英文）包含由空白資料庫到選用匯入和
AI 設定的完整步驟。
...
或產品名稱，以及直接匯入的欄位會保留原文。AI 為保存內容撰寫文字時，會使用

...
平时可以直接用适合手机操作的网页界面记录；如果想用照片或几句自然语言完成
记录，也可以连接 AI 助手。固定而重复的数据则可以选择通过 iPhone 快捷指令或
API 自动传入。AI 和自动化功能都是可选项，网页界面本身即可独立运行。
...
2. **AI 助手（可选）：** 适合处理照片、自然语言汇报、修改、提问，以及根据记录
   提供建议。
3. **自动化（可选）：** 通过 iPhone 快捷指令或 API 传入特定、固定而重复的数据。
...
[新用户入门指南](docs/ONBOARDING.md)（英文）包含从空数据库开始，到可选自动化和
AI 设置的完整步骤。
...
- SQLite 数据库是正式记录。
- 网页登录、AI 访问和自动化／API 访问分别使用不同的凭据。
...
以及通过 API 传入的字段会保留原文。AI 为保存内容撰写文字时，会使用个人资料中设置的
